...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, stage_idx):
        if self.task_name == 'forecast':
            dec_out = self.forecast(x_enc, x_mark_enc, stage_idx)
            return dec_out
        return None

    def forecast(self, x_enc, x_mark_enc, stage_idx):
        B, L, M = x_enc.shape
        ...
        for b in range(x_enc.shape[0]):
            ...
            if self.configs.data_path == 'abilene_tm.csv':
                prompt_ = (
                    ...
                    f"[Instruction]: Learn spatiotemporal patterns in traffic flows, perform progressive matrix forecast through six refinement stages:\n"
               ...
               prompt_ = (
                    ...
                    f"During the training process, we select fixed-interval rows as sampled data.\n"
                    f"[Instruction]: Learn spatiotemporal patterns in traffic flows. Perform progressive matrix completion through exponential forecast.\n"
